...
class Widget extends MX_Controller {
	...
	function box()
	{
		...
		$this->load->view('widget/box');
	}


	function box_open($title, $style = 'primary', $solid = FALSE, $buttonname = '', $buttonurl ='#')
	{
		$data = array(
			'title'		=> $title,
			'style'		=> empty($style) ? '' : 'box-'.$style,
	...
	function box_body($body, $count = '')
	{
		$data = array(
			'body'		=> $body,
			'count'		=> $count,
		);
		$this->load->view('widget/box_body', $data);
	}
	...
	function collapsable()
	{
		...
		$this->load->view('widget/collapsable');
	}
	function tooltipsonbutton()
	{
	
		$this->load->view('widget/tooltipsonbutton');
	}
}